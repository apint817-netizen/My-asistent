...
export default function AnalysisView() {
    ...
    // Initial greeting if empty — check store directly to avoid StrictMode double-fire
    useEffect(() => {
        const currentMessages = useStore.getState().analysisMessages;
    ...
    const generateAIResponse = async (userMessage) => {
        ...
        try {
            ...
            const systemInstruction = `Ты Стратег Nova — ИИ-коуч и планировщик. Отвечай ТОЛЬКО по-русски.

🎯 ТВОЙ СТИЛЬ:
- Деловой, но дружелюбный. Никакой воды — только конкретика
- Задавай 1-2 точных вопроса, потом СРАЗУ предлагай задачи через теги
- Используй эмодзи умеренно для структуры (📋, ✅, 🎯, ⏰)
- Всегда привязывай советы к профилю и целям пользователя

📋 АЛГОРИТМ РАБОТЫ:
1. Пользователь описывает ситуацию → задай 1-2 уточняющих вопроса
2. После ответа → СРАЗУ предложи 3-5 задач и 1-2 награды через теги черновика
3. Отклоняют → спроси "Почему?" и предложи альтернативу
4. Принимают → похвали и спроси, есть ли ещё дела
${isProfileEmpty ? '\n⚠️ ПРОФИЛЬ ПУСТ! Перед планированием мягко спроси: "Чтобы составить точный план, расскажи коротко — чем занимаешься и какие у тебя главные цели?"' : ''}

📅 Сегодня: ${todayDate}. Очки задач: 5-100. Стоимость наград: 20-500.

📊 КОНТЕКСТ:
- 🔥 Стрик: ${useStore.getState().streak} дней
- 💰 Очки: ${tokens}
- ✅ Выполнено: ${completedTasks}
- 📝 Невыполненные: ${pendingTasks}
- 🎁 Награды: ${availableRewards}
- 📅 Календарь: ${calendarStr}

👤 ПРОФИЛЬ:
- Кто: ${userProfile.bio || '❌ Не заполнено'}
- Цели: ${userProfile.goals || '❌ Не заполнено'}
- Интересы: ${userProfile.interests || '❌ Не заполнено'}

🏷️ ТЕГИ ЧЕРНОВИКА (добавляют в план справа):
1. [TASK: "Название" | Очки]
2. [CALENDAR_TASK: "Название" | Очки | YYYY-MM-DD]
3. [HABIT: "Название" | Очки | "Период"]
4. [REWARD: "Название" | Стоимость]

⚡ ТЕГИ ПРЯМОГО УПРАВЛЕНИЯ:
- [COMPLETE_TASK: "ref"] · [EDIT_TASK_POINTS: "ref" | очки]
- [DELETE_TASK: "ref"] · [BUY_REWARD: "ref"]

После тегов черновика скажи: "Я добавил план в черновик справа 👉 Посмотри и скажи, всё ли ок?".`;
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-2 lg:gap-6 flex-1 h-full lg:min-h-[500px]">
            ...
            <div className={`glass-panel p-0 flex-col h-[65vh] min-h-[400px] lg:h-full lg:min-h-0 overflow-hidden border-accent/20 shrink-0 ${mobileTab === 'chat' ? 'flex' : 'hidden lg:flex'}`}>
                ...
                <div className="px-4 pb-2 flex flex-wrap gap-2">
                    {[
                        { text: '📊 Оценка продуктивности', msg: 'Оцени мою продуктивность за сегодня. Учти стрик, очки, выполненные задачи. Дай честную оценку по 10-балльной шкале с конкретными советами что улучшить.' },
                        { text: '📝 План на сегодня', msg: 'Составь мне оптимальный план на сегодня. Учти невыполненные задачи и календарь. Предложи 3-5 конкретных задач через теги [TASK] с адекватными очками, и 1 награду через [REWARD].' },
                        { text: '🗓 План на неделю', msg: 'Помоги распланировать неделю. Предложи задачи на каждый день через [CALENDAR_TASK] и 2-3 привычки через [HABIT]. Основывайся на моём профиле и целях.' },
                        { text: '🎁 Идеи наград', msg: 'Предложи 5 персонализированных наград на основе моих интересов и хобби. Используй теги [REWARD] с адекватной стоимостью (от 30 до 300 очков).' },
                        { text: '🏃 Утренняя рутина', msg: 'Создай мне утреннюю рутину из 3-5 полезных привычек. Используй теги [HABIT] с периодом EVERY_DAY или WORK_DAYS. Учти мой профиль.' },
                        { text: '💥 Завал задач', msg: 'У меня накопилось много дел. Помоги расставить приоритеты — что сделать первым, что можно перенести на завтра, а что удалить. Будь конкретным.' },
                        { text: '⚖️ Аудит баланса', msg: 'Проверь баланс: справедливы ли цены моих наград относительно очков за задачи? Дай 1-2 конкретные рекомендации по корректировке через теги.' }
                    ].map((btn, i) => (
                        <button
                            key={i}
                            onClick={() => {
